# 30-round converged comparison on representative subset
import csv, os, time
import numpy as np
import torch
...
NUM_CLIENTS = 20
NUM_ROUNDS = 30                      # the whole point: past the cache warm-up
METHODS = ["FedAvg", "Median", "Krum", "Bulyan", "FLTrust", "SWB", "SWB-DM"]
CONDITIONS = [("none", 0.0), ("alie", 0.3), ("ipm", 0.3)]
PARTICIPATIONS = [0.5, 0.1]
SEEDS = [42, 7]
RESULTS_FILE = "grid_results_30r.csv"
NUM_WORKERS = 6
_cache = {}
    ...
    try:
        ...
        for r in range(1, NUM_ROUNDS + 1):
            ...
            if mal_ids and attack != "none":
                ...
                mal_counts = [int(np.mean(ben_counts))] * len(mal_states) if ben_counts else [counts[c] for c in mal_ids]
            elif mal_ids:
                for cid in mal_ids:
                    worker.load_state_dict(current)
                    st = train_client(worker, loaders[cid], epochs=2, lr=0.01)
...
if __name__ == "__main__":
    ...
    todo = [(m, a, c, p, s) for m in METHODS for (a, c) in CONDITIONS
            for p in PARTICIPATIONS for s in SEEDS if (m, a, c, p, s) not in done]
    ...
    with ProcessPoolExecutor(max_workers=NUM_WORKERS, initializer=_worker_init) as ex:
        ...
        for i, fut in enumerate(as_completed(futs)):
            ...
            print(f"[{i+1:2d}/{len(todo)}] {cfg[0]:<9} | {cfg[1]:<5} | cor={cfg[2]:<3} | part={cfg[3]:<3} | s={cfg[4]} | {acc:6.2f}% | "
                  f"ETA {(len(todo)-i-1)/rate if rate else -1:.0f}m", flush=True)
    print("30-ROUND EXTENSION COMPLETE", flush=True)